...
Polloc::Typing::bandingPattern::amplification - banding-pattern-based
...
L<Polloc::Typing::bandingPattern>
...
package Polloc::Typing::bandingPattern::amplification;

use strict;
use Polloc::Polloc::IO;
use Polloc::LocusI;
use base qw(Polloc::Typing::bandingPattern);
...
=head1 METHODS FROM Polloc::Typing::bandingPattern
...
sub fragments {
   ...
   for my $g (0 .. $#{$locigroup->genomes}){
      ...
      while(my $ln = $run->_readline){
         chomp $ln;
	 if($ln =~ m/^Amplimer (\d+)/){ $amp->[$k = $1-1] = Polloc::LocusI->new(-type=>'amplicon', -primersio=>$primers) }
...
A <Polloc::Polloc::IO> object pointing to a file containing the primers
...
sub design_primers {
   ...
   # Output file
   my $io = Polloc::Polloc::IO->new(-createtemp=>1);
