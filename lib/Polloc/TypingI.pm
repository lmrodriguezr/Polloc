...
Polloc::TypingI - Generic typing interface

=head1 DESCRIPTION

Use this interface to initialize the Polloc::Typing::* objects.  Any
rule inherits from this Interface.  Usually, rules are initialized
from files (via the L<Polloc::TypingIO> package).
...
Attempts to initialize a C<Polloc::Typing::*> object
...
=item -locigroup I<Polloc::LociGroup object>

Group of loci (L<Polloc::LociGroup>) to be use for typing.

=back

=head3 Returns

The C<Polloc::Typing::*> object
...
sub new {
   ...
   if($class !~ m/Polloc::Typing::(\S+)/){
      my $bme = Polloc::Polloc::Root->new(@args);
      my($type) = $bme->_rearrange([qw(TYPE)], @args);
      
      if($type){
         $type = Polloc::TypingI->_qualify_type($type);
         $class = "Polloc::Typing::" . $type if $type;
      }
   }

   # Try to load the object
   if($class =~ m/Polloc::Typing::(\S+)/){
      if(Polloc::TypingI->_load_module($class)){;
   ...
   my $bme = Polloc::Polloc::Root->new(@args);
   $bme->throw("Impossible to load the proper Polloc::TypingI class with ".
   		"[".join("; ",@args)."]", $class);
}
...
Value (str).  See L<new()> and the corresponding C<Polloc::Typing::*>
...
A L<Polloc::LociGroup> object (optional).

=head3 Returns

A L<Polloc::LociGroup> object or C<undef>.
...
throw L<Polloc::Polloc::NotImplementedException> if not
implemented.
...
A L<Polloc::LociGroup> object containing the actual loci
employed for typing.

=cut

sub scan { $_[0]->throw("scan", $_[0], "Polloc::Polloc::NotImplementedException") }
...
sub cluster { $_[0]->throw("cluster", $_[0], "Polloc::Polloc::NotImplementedException") }
...
=item -loci I<Array of Polloc::LocusI>

The loci to be evaluated.  Note that it is a reference array
of L<Polloc::LocusI> objects, and B<NOT> a L<Polloc::LociGroup>.
...
sub typing_value {
   $_[0]->throw("typing_value", $_[0], "Polloc::Polloc::NotImplementedException")
}
