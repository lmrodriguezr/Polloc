...
# 17
my $G = [Polloc::Genome->new(-file=>'t/multi.fasta'),
	Polloc::Genome->new(-file=>'t/repeats.fasta')];
$T->genomes($G);
...
isa_ok($L->loci->[0]->genome, 'Polloc::Genome');

# 30
my $crit = $T->groupcriteria->[0];
isa_ok($crit->locigroup($L), 'Polloc::LociGroup');
is($#{$crit->get_loci}, 1);
...
isa_ok($groups, 'ARRAY');
is($#$groups, 0);
isa_ok($groups->[0], 'Polloc::LociGroup');
is($#{$groups->[0]->loci}, 1);
...
isa_ok($groups->[0]->genomes->[0], 'Polloc::Genome');

# 39
my $nL = $crit->extend(-loci=>$groups->[0]);
isa_ok($nL, 'Polloc::LociGroup');
# Genomes passed from $groups->[0] to $nL:
isa_ok($nL->genomes, 'ARRAY');
isa_ok($nL->genomes->[0], 'Polloc::Genome');

# 42
is($#{$nL->loci}, 1);
isa_ok($nL->loci->[0]->genome, 'Polloc::Genome');
